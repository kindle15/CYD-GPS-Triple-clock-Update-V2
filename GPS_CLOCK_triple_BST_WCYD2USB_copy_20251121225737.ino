...
************************************************************************************************
 ...
 PORTED BY KINDLE15 GITHUB
                NOVEMBER 2025 Minor rewrite to use CYD2USB (2432S028R) Module spacifically.
                GPS Tx and PPS lines, Power & GND: No Change
                Screen, removed uncompatable screen and instructions and installed relevent drivers:
                XPT2046_Touchscreen_V1.4(R) and initiated the reversed screen (need a mirror otherwise).
                This rewrite is the update and addition (see next section for details) to
                Coolmdxi update September 2024.

                ADDITION: Remove static timezone instruction. Insert Dynamic timezone instruction using rough
                longitudes to determine timezone, (installed all US wide zones). This is rough due to the extreme 
                amounts of datasets required to follow all timezones and their deviations. There will be position 
                and either early or late trigger errors because of this method. Also repaired related InRegion 
                assosiated errors (identifying as might have issuses later).
...
#include <XPT2046_Touchscreen.h>  //https://github.com/bmorcelli/Launcher/tree/main/lib/CYD-touch
#include <Wire.h>                 
#include <SPI.h>                  
...
// time‑zone rules – placeholders for the four U.S. zones
...
// pointer to currently selected rule (set by lookup) and the Timezone
...
#define SCREEN_ORIENTATION 1  // landscape mode, should be '1' or '3' 3 usb Left
#define TOUCH_FLIP_X true    // touch: left-right orientation //unused as touch has own rotation
...
boolean inRegion(const region *b, int x, int y) {  // true if region contains point (x,y)
